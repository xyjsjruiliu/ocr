...
from ocr.data_loader import get_transforms
from ocr.models import build_model
from ocr.post_processing import get_post_processing
import numpy as np
import glob
...
def init_args():
    ...
    parser.add_argument('--model_path', default=r'ocr/output/DBNet_resnet18_FPN_DBHead/checkpoint/model_best.pth', type=str)
    # parser.add_argument('--data', default='./imgs/demo.jpg', type=str, help='img path or folder for predict')
    parser.add_argument('--data', default='ocr/imgs/8e476594583b496cbe630023e464fcde.jpg', type=str, help='img path or folder for predict')
...
if __name__ == '__main__':
    ...
    if os.path.isfile(data_f) and 'jpg' in data_f:
        ...
        for data_f in img_files:
            ...
            img = cv2.imread(data_f)
            preds, boxes_list, score_list, t = model.predict(data_f, is_output_polygon=args.polygon)
            print(preds, t, score_list)
            
            for b in boxes_list:
                rects = np.int32(np.array(b).reshape(-1, 2))
                res = cv2.polylines(img, [rects], True, (0, 255, 0), 2, cv2.LINE_AA)
            # cv2.imshow('aa', res)
            cv2.imwrite("cat2.jpg", res)
            cv2.waitKey(0)
